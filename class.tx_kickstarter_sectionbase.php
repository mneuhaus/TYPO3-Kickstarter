...
class tx_kickstarter_sectionbase {
	...
	function renderCheckBox($prefix, $value, $defVal = 0) {
		...
		return $this->wopText($prefix) . $onCP[0] . '<input type="hidden" name="' . $this->piFieldName('wizArray_upd')
		. $prefix . '" value="0"><input type="checkbox" class="checkbox" id="field_' . md5($prefix) . '" name="'
		. $this->piFieldName("wizArray_upd") . $prefix . '" value="1"' . ($value ? ' checked="checked"' : '')
		. ' onclick="' . $onCP[1] . '"' . $this->wop($prefix) . '>';
	}
	...
	function renderTextareaBox($prefix, $value) {
		$onCP = $this->getOnChangeParts($prefix);
		return $this->wopText($prefix) . $onCP[0] . '<textarea name="' . $this->piFieldName('wizArray_upd') . $prefix
		. '" style="width:600px;" rows="10" wrap="off" onchange="' . $onCP[1] . '" title="' . htmlspecialchars(
			'WOP:' . $prefix
		) . '"' . $this->wop($prefix) . '>' . t3lib_div::formatForTextarea($value) . '</textarea>';
	}
	...
	function renderStringBox($prefix, $value, $width = 200) {
		$onCP = $this->getOnChangeParts($prefix);
		return $this->wopText($prefix) . $onCP[0] . '<input type="text" name="' . $this->piFieldName('wizArray_upd')
		. $prefix . '" value="' . htmlspecialchars($value) . '" style="width:' . $width . 'px;" onchange="' . $onCP[1]
		. '"' . $this->wop($prefix) . '>';
	}
	...
	function renderRadioBox($prefix, $value, $thisValue) {
		$onCP = $this->getOnChangeParts($prefix);
		return $this->wopText($prefix) . $onCP[0] . '<input type="radio" name="' . $this->piFieldName('wizArray_upd')
		. $prefix . '" value="' . $thisValue . '"' . (!strcmp($value, $thisValue) ? ' checked="checked"' : '')
		. ' onclick="' . $onCP[1] . '"' . $this->wop($prefix) . '>';
	}
	...
	function renderSelectBox($prefix, $value, $optValues) {
		...
		if (!$isSelFlag && strcmp('', $value)) {
			$opt[] =
				'<option value="' . $value . '" selected="selected">' . htmlspecialchars(
					"CURRENT VALUE '" . $value . "' DID NOT EXIST AMONG THE OPTIONS"
				) . '</option>';
		}
		return $this->wopText($prefix) . $onCP[0] . '<select name="' . $this->piFieldName("wizArray_upd") . $prefix
		. '" onchange="' . $onCP[1] . '"' . $this->wop($prefix) . '>' . implode('', $opt) . '</select>';
	}
	...
	function whatIsThis($str) {
		return ' <a href="#" title="' . htmlspecialchars($str) . '" style="cursor:help" onclick="alert('
		. $GLOBALS['LANG']->JScharCode($str) . ');return false;">(What is this?)</a>';
	}
	...
	function renderStringBox_lang($fieldName, $ffPrefix, $piConf) {
		...
		if (count($this->wizard->selectedLanguages)) {
			...
			foreach ($this->wizard->selectedLanguages as $k => $v) {
				$lines[] = $this->renderStringBox(
						$ffPrefix . '[' . $fieldName . '_' . $k . ']', $piConf[$fieldName . '_' . $k]
					) . ' [' . $v . ']';
			}
	...
	function textSetup($header, $content) {
		return ($header ? '<strong>' . $header . '</strong><br />' : '') . '<blockquote>' . trim($content)
		. '</blockquote>';
	}
	...
	function resImg($name, $p = 'align="center"', $pre = '<br />', $post = '<br />') {
		...
		return $pre . '<img src="' . $this->wizard->siteBackPath . $imgRel . '" ' . $imgInfo[3] . ($p ? ' ' . $p : '')
		. ' vspace="5" border="1" style="border:solid 1px;" />' . $post;
	}
	...
	function wopText($prefix) {
		return $this->printWOP ?
			'<font face="verdana,arial,sans-serif" size=1 color="#999999">' . htmlspecialchars($prefix)
			. ':</font><br />' : '';
	}
	...
	function linkCurrentItems($cat) {
		...
		if (is_array($items)) {
			foreach ($items as $k => $conf) {
				$lines[] = '<strong>' . $this->linkStr(
						$conf['title'] ? $conf['title'] : '<em>Item ' . $k . '</em>', $cat, 'edit:' . $k
					) . '</strong>';
				$c = $k;
			}
		}
	...
	function cmdHiddenField() {
		return '<input type="hidden" name="' . $this->piFieldName('cmd') . '" value="' . htmlspecialchars(
			$this->currentCMD
		) . '">';
	}
	...
	function fieldIsRTE($fC) {
		...
		&& ($fC['conf_rte'] == 'basic'
			|| (t3lib_div::inList('custom,moderate', $fC['conf_rte'])
				&& $fC['conf_mode_cssOrNot'])
		);
	}
...
######### Functions from compilefiles #########
	...
	function writeStandardBE_xMod($extKey, array $config, $pathSuffix, $cN, $k, $k_prefix) {
		...
		$this->addLocalLangFile(
			$ll, $pathSuffix . 'locallang.xml', 'Language labels for ' . $extKey . ' module ' . $k_prefix . $k
		);

		// Add clear.gif
		$this->addFileToFileArray(
			$pathSuffix . 'clear.gif', t3lib_div::getUrl(t3lib_extMgm::extPath('kickstarter') . 'res/clear.gif')
		);
		...
		switch((string) $this->MOD_SETTINGS[\'function\'])	{
			case 1:
				$content = \'<div align=center><strong>Hello World!</strong></div><br />
					The "Kickstarter" has made this module automatically, it contains a default framework for a backend module but apart from that it does nothing useful until you open the script "\'.substr(t3lib_extMgm::extPath(\''
			. $extKey . '\'),strlen(PATH_site)).$pathSuffix.index.php.\'" and edit it!
					<hr />
	 ...
	 * @return    [type]        ...
	 */
	function addLocalConf(
		&$lArray, $confArray, $key, $prefix, $subPrefix, $dontPrefixKey = 0, $noWOP = 0, $overruleKey = ''
	) {
		reset($this->wizard->languages);

		$overruleKey = $overruleKey ? $overruleKey : ($dontPrefixKey ? '' : $prefix . $subPrefix . '_') . $key;

		$lArray['default'][$overruleKey] =
			array($confArray[$key], (!$noWOP ? 'WOP:[' . $prefix . '][' . $subPrefix . '][' . $key . ']' : ''));
		...
		$file .= trim(
			$this->sPS(
				'
							...
							if (defined(\'TYPO3_MODE\') && isset($GLOBALS[\'TYPO3_CONF_VARS\'][TYPO3_MODE][\'XCLASS\'][\'ext/' . $extKey
				. '/' . $filename . '\'])) {
	...
	function addFileToFileArray($name, $content, $mode = 0) {
		switch ($mode) {
			case 1: // Append
				$this->wizard->fileArray[$name] =
					$this->makeFileArray($name, $this->wizard->fileArray[$name]['content'] . chr(10) . $content);
				break;
			case -1: // Prepend
				$this->wizard->fileArray[$name] =
					$this->makeFileArray($name, $content . chr(10) . $this->wizard->fileArray[$name]['content']);
				break;
	...
	function makeEMCONFpreset($prefix = '') {
		...
		$EM_CONF = array();
		$presetFields = explode(
			',',
			'title,description,category,shy,dependencies,conflicts,priority,module,state,internal,uploadfolder,createDirs,modify_tables,clearCacheOnLoad,lockType,author,author_email,author_company,private,download_password,version'
		);
		...
		if (is_array($this->wizard->EM_CONF_presets['modify_tables'])) {
			$EM_CONF[$prefix . 'modify_tables'] =
				implode(',', array_unique($this->wizard->EM_CONF_presets['modify_tables']));
		}
	...
	function userField($k) {
		$v = '';
		if ($k == 'name') {
			$v = ($this->wizard->wizArray['emconf'][1]['author'] != '') ? $this->wizard->wizArray['emconf'][1]['author']
				: $GLOBALS['BE_USER']->user['realName'];
		} else {
			if ($k == 'email') {
				$v = ($this->wizard->wizArray['emconf'][1]['author_email'] != '')
					? $this->wizard->wizArray['emconf'][1]['author_email'] : $GLOBALS['BE_USER']->user['email'];
			}
		}
		return $v;
	}
...
if (defined('TYPO3_MODE')
	&& $TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/kickstarter/class.tx_kickstarter_sectionbase.php']
) {
