	...
	function makeFilesArray($extKey) {
		...
		if (count($this->ext_locallang)) {
			$this->addLocalLangFile(
				$this->ext_locallang, 'locallang.xml', 'Language labels for extension \'' . $extKey . '\''
			);
		}
		// Local lang DB file:
		if (count($this->ext_locallang_db)) {
			$this->addLocalLangFile(
				$this->ext_locallang_db, 'locallang_db.xml',
				'Language labels for database tables/fields belonging to extension \'' . $extKey . '\'', 'database'
			);
		}
...
'
		...
		// icon:
		$this->addFileToFileArray(
			'ext_icon.gif', t3lib_div::getUrl(t3lib_extMgm::extPath('kickstarter') . 'res/notfound.gif')
		);
	}
...
// Include extension?
if (defined('TYPO3_MODE')
	&& $TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/kickstarter/class.tx_kickstarter_compilefiles.php']
) {
