...
class tx_kickstarter_section_cm extends tx_kickstarter_sectionbase {
	...
	function render_wizard() {
		...
		if ($action[0] == 'edit') {
			...
			$subContent .= $this->renderCheckBox(
					$ffPrefix . '[second_level]', $piConf['second_level']
				) . 'Activate a second-level menu.<br />';
			$subContent .= $this->renderCheckBox(
					$ffPrefix . '[only_page]', $piConf['only_page']
				) . 'Add only if the click menu is on a \'Page\' (example)<br />';
			$subContent .= $this->renderCheckBox(
					$ffPrefix . '[only_if_edit]', $piConf['only_if_edit']
				) . 'Only active if item is editable.<br />';
			$subContent .= $this->renderCheckBox(
					$ffPrefix . '[remove_view]', $piConf['remove_view']
				) . 'Remove \'Show\' element (example)<br />';
			$lines[] = '<tr' . $this->bgCol(3) . '><td>' . $this->fw($subContent) . '</td></tr>';
		}
	...
	function render_extPart($k, $config, $extKey) {
		...
		if ($config['second_level']) {
			...
			$this->addLocalConf(
				$this->wizard->ext_locallang, array('title_activate' => '...Second level ->'), 'title_activate', 'cm',
				$k, 0, 1
			);
			// Add activate icon
			$this->addFileToFileArray(
				$pathSuffix . 'cm_icon_activate.gif',
				t3lib_div::getUrl(t3lib_extMgm::extPath('kickstarter') . 'res/notfound_module.gif')
			);
		}
		...
		if ($config['remove_view']) {
			$content .= $this->sPS(
				'
								// Removes the view-item from clickmenu  ' . $this->WOPcomment(
					'(WOP:' . $WOP . '[remove_view])'
				) . '
...
if (defined('TYPO3_MODE')
	&& $TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/kickstarter/sections/class.tx_kickstarter_section_cm.php']
) {
