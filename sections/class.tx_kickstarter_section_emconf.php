	...
	function render_wizard() {
		...
		if ($action[0] == 'edit') {
			...
			$subContent = '<strong>Author Name:</strong><br />' .
				$this->renderStringBox(
					$ffPrefix . '[author]',
					$piConf['author'] ? $piConf['author'] : $GLOBALS['BE_USER']->user['realName']
				);
			$lines[] = '<tr' . $this->bgCol(3) . '><td>' . $this->fw($subContent) . '</td></tr>';
			...
			$subContent = '<strong>Author email:</strong><br />' .
				$this->renderStringBox(
					$ffPrefix . '[author_email]',
					$piConf['author_email'] ? $piConf['author_email'] : $GLOBALS['BE_USER']->user['email']
				);
			$lines[] = '<tr' . $this->bgCol(3) . '><td>' . $this->fw($subContent) . '</td></tr>';
		}
...
if (defined('TYPO3_MODE')
	&& $TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/kickstarter/sections/class.tx_kickstarter_section_emconf.php']
) {
