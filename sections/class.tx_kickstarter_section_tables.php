...
class tx_kickstarter_section_tables extends tx_kickstarter_section_fields {
		...
		switch ($fConf['type']) {
			...
			case 'input+':
				...
				if ($fConf['conf_unique']) {
					$details[] = ($fConf['conf_unique'] == 'L') ? 'unique (page)' : 'unique (site)';
				}
				if ($fConf['conf_eval']) {
					$details[] = $optEval[$fConf['conf_eval']];
				}
			...
			case 'check_4':
				...
				break;
			case 'radio':
				if ($fConf['conf_select_items']) {
					$details = $fConf['conf_select_items'] . ' options';
				}
				break;
			case 'select':
				if ($fConf['conf_select_items']) {
	...
	function render_extPart($k, $config, $extKey) {
		...
		$ctrl[] = trim(
			$this->sPS(
				'
							\'title\'     => \'' . $this->getSplitLabels_reference($config, 'title', $tableName) . '\',		'
				. $this->WOPcomment('WOP:' . $WOP . '[title]') . '
			\'label\'     => \'' . ($config['header_field'] ? $config['header_field'] : 'uid') . '\',	'
				. $this->WOPcomment('WOP:' . $WOP . '[header_field]') . '
			\'tstamp\'    => \'tstamp\',
			\'crdate\'    => \'crdate\',
			\'cruser_id\' => \'cruser_id\',
		', 0
		...
		if ($config['type_field']) {
			$ctrl[] = '\'type\' => \'' . $config['type_field'] . '\',	' . $this->WOPcomment(
					'WOP:' . $WOP . '[type_field]'
				);
		}
		...
		if ($config['localization']) {
			$ctrl[] = '\'languageField\'            => \'sys_language_uid\',	' . $this->WOPcomment(
					'WOP:' . $WOP . '[localization]'
				);
			$ctrl[] = '\'transOrigPointerField\'    => \'l10n_parent\',	' . $this->WOPcomment(
					'WOP:' . $WOP . '[localization]'
				);
			$ctrl[] = '\'transOrigDiffSourceField\' => \'l10n_diffsource\',	' . $this->WOPcomment(
					'WOP:' . $WOP . '[localization]'
				);
		...
		if ($config['sorting']) {
			...
			$ctrl[] = '\'default_sortby\' => \'ORDER BY ' . trim(
					$config['sorting_field'] . ' ' . ($config['sorting_desc'] ? 'DESC' : '')
				) . '\',	' . $this->WOPcomment(
					'WOP:' . $WOP . '[sorting] / ' . $WOP . '[sorting_field] / ' . $WOP . '[sorting_desc]'
				);
		}
		...
		if (is_array($enFields) && count($enFields)) {
			$ctrl[] = trim(
				$this->wrapBody(
					'
									\'enablecolumns\' => array(		' . $this->WOPcomment(
						'WOP:' . $WOP . '[add_hidden] / ' . $WOP . '[add_starttime] / ' . $WOP . '[add_endtime] / '
						. $WOP . '[add_access]'
					) . '
		...
		$this->addFileToFileArray(
			'icon_' . $tableName . '.gif',
			t3lib_div::getUrl(t3lib_extMgm::extPath('kickstarter') . 'res/' . $config['defIcon'])
		);
