...
class tx_kickstarter_section_pi extends tx_kickstarter_sectionbase {
	...
	function render_wizard() {
		...
		if ($action[0] == 'edit') {
			...
			$subContent = $this->renderCheckBox(
					$ffPrefix . '[plus_user_obj]', $piConf['plus_user_obj']
				) . 'By default plugins are generated as cachable USER cObjects. Check this checkbox to generate an uncached USER_INT cObject.<br />';
			$lines[] = '<tr' . $this->bgCol(3) . '><td>' . $this->fw($subContent) . '</td></tr>';

			$subContent = $this->renderCheckBox(
					$ffPrefix . '[plus_not_staticTemplate]', $piConf['plus_not_staticTemplate']
				) . 'Enable this option if you want the TypoScript code to be set by default. Otherwise the code will go into a static template file which must be included in the template record (it\'s <em>NOT</em> recommended to set this option).<br />';
			$lines[] = '<tr' . $this->bgCol(3) . '><td>' . $this->fw($subContent) . '</td></tr>';
			...
			if (is_array($this->wizard->wizArray['fields'])) {
				...
				foreach ($this->wizard->wizArray['fields'] as $kk => $fC) {
					if ($fC['which_table'] == 'tt_content') {
						$optValues[$kk] =
							($fC['title'] ? $fC['title'] : 'Item ' . $kk) . ' (' . count($fC['fields']) . ' fields)';
					}
				}
			...
			if (is_array($this->wizard->wizArray['tables'])) {
				...
				foreach ($this->wizard->wizArray['tables'] as $kk => $fC) {
					$optValues[$kk] = ($fC['tablename'] || $fC['title'] ?
							$fC['title'] . ' (' . $this->returnName($this->wizard->extKey, 'tables') . ($fC['tablename']
								? '_' . $fC['tablename'] : '') . ')' : 'Item ' . $kk) . ' (' . count(
							$fC['fields']
						) . ' fields)';
				}
				...
				$incListing .= '<br />' . $this->renderCheckBox(
						$ffPrefix . '[list_default_listmode]', $piConf['list_default_listmode']
					) .
					'Listing: Sections instead of table-rows';
				$incListing .= '<br />' . $this->renderCheckBox(
						$ffPrefix . '[list_default_singlemode]', $piConf['list_default_singlemode']
					) .
			...
			$subContent = $this->renderRadioBox($ffPrefix . '[addType]', $piConf['addType'], 'includeLib') .
				$this->textSetup(
					...
					$this->renderCheckBox($ffPrefix . '[plus_user_ex]', $piConf['plus_user_ex'])
					. 'Provide TypoScript example for USER cObject in \'page.1000\'<br />'
				);
			$lines[] = '<tr' . $this->bgCol(3) . '><td>' . $this->fw($subContent) . '</td></tr>';
		}
		...
		switch ($config['addType']) {
			case 'list_type':
				...
				$this->wizard->ext_tables[] = $this->sPS(
					...
					" . ($config['apply_extended'] ?
						"\$TCA['tt_content']['types']['list']['subtypes_addlist'][\$_EXTKEY . '_pi" . $k . "'] = '"
						. $this->wizard->_apply_extended_types[$config['apply_extended']] . "';" : "") . "
				"
				);
			...
			case 'textbox':
				...
				if ($config['apply_extended']) {
					$this->wizard->ext_tables[] = $this->sPS(
						...
						\$TCA['tt_content']['types']['splash']['subtypes_addlist'][\$_EXTKEY.'_pi" . $k . "']='"
						. $this->wizard->_apply_extended_types[$config['apply_extended']] . "';
					"
					);
				}
			...
			case 'menu_sitemap':
				...
				if ($config['apply_extended']) {
					$this->wizard->ext_tables[] = $this->sPS(
						...
						\$TCA['tt_content']['types']['menu']['subtypes_addlist'][\$_EXTKEY.'_pi" . $k . "']='"
						. $this->wizard->_apply_extended_types[$config['apply_extended']] . "';
					"
					);
				}
		...
		if ($setType && !t3lib_div::inList('typotags,includeLib', $setType)) {
			$this->wizard->ext_tables[] = $this->sPS(
				...
				t3lib_extMgm::addPlugin(array(
					'" . addslashes(
					$this->getSplitLabels_reference($config, 'title', 'tt_content.' . $setType . '_pi' . $k)
				) . "',
					\$_EXTKEY . '_pi" . $k . "',
			...
			case 'list_type':
				if ($config['list_default']) {
					if (is_array($this->wizard->wizArray['tables'][$config['list_default']])) {
						...
						$tcol = 'uid';
						$theLines['getListRow'][$tcol] =
							'<td><p>\' . $this->getFieldContent(\'' . $tcol . '\') . \'</p></td>';
						$theLines['getListHeader'][$tcol] =
							'<td><p>\' . $this->getFieldHeader_sortLink(\'' . $tcol . '\') . \'</p></td>';
						...
						if (is_array($tempTableConf['fields'])) {
							...
							while (list(, $fC) = each($tempTableConf['fields'])) {
								$tcol = $fC['fieldname'];
								if ($tcol) {
									$theLines['singleRows'][$tcol] = trim(
										$this->sPS(
											'
																					<tr>
																						<td nowrap="nowrap" valign="top"\' . $this->pi_classParam(\'singleView-HCell\') . \'><p>\' . $this->getFieldHeader(\''
											. $tcol . '\') . \'</p></td>
									...
									if ($this->fieldIsRTE($fC)) {
										$theLines['singleRows_section'][$tcol] = trim(
											$this->sPS(
												'
										...
										$theLines['singleRows_section'][$tcol] = trim(
											$this->sPS(
												'
																							<p\' . $this->pi_classParam("' . $tempN
												. '") . \'><strong>\' . $this->getFieldHeader(\'' . $tcol
												. '\') . \':</strong> \' . $this->getFieldContent(\'' . $tcol . '\') . \'</p>
										'
											)
										);
										$P_classes['SV'][] = $tempN;
									...
									if (!strstr($fC['type'], 'textarea')) {
										$theLines['getListRow'][$tcol] =
											'<td valign="top"><p>\' . $this->getFieldContent(\''
											. $tcol . '\') . \'</p></td>';
										$theLines['getListHeader'][$tcol] =
											'<td nowrap="nowrap"><p>\' . $this->getFieldHeader(\''
											. $tcol . '\') . \'</p></td>';

										$tempN = 'listrowField-' . str_replace('_', '-', $tcol);
										$theLines['listItemRows'][$tcol] = trim(
											$this->sPS(
												'
																							<p\' . $this->pi_classParam(\'' . $tempN
												. '\') . \'>\' . $this->getFieldContent(\'' . $tcol . '\') . \'</p>
										'
											)
										);
										$P_classes['LV'][] = $tempN;
									...
									$this->addLocalConf(
										$ll, array('listFieldHeader_' . $tcol => $fC['title']),
										'listFieldHeader_' . $tcol, 'pi', $k, 1, 1
									);

									if ($tcol === 'title') {
										$theLines['getFieldContent'][$tcol] = trim(
											$this->sPS(
												'
																								case "' . $tcol . '":
														// This will wrap the title in a link.
													return $this->pi_list_linkSingle($this->internal[\'currentRow\'][\''
												. $tcol . '\'], $this->internal[\'currentRow\'][\'uid\'], 1);
													break;
										'
											)
										);
										$theLines['getFieldHeader'][$tcol] = trim(
											$this->sPS(
												'
																								case "' . $tcol . '":
													return $this->pi_getLL(\'listFieldHeader_' . $tcol . '\', \'<em>'
												. $tcol . '</em>\');
													break;
										'
											)
										...
										$theLines['getFieldContent'][$tcol] = trim(
											$this->sPS(
												'
																									case "' . $tcol . '":
														return $this->pi_RTEcssText($this->internal[\'currentRow\'][\''
												. $tcol . '\']);
														break;
											'
											)
										...
										$theLines['getFieldContent'][$tcol] = trim(
											$this->sPS(
												'
																								case "' . $tcol . '":
													return strftime(\'%d-%m-%y %H:%M:%S\', $this->internal[\'currentRow\'][\''
												. $tcol . '\']);
													break;
										'
											)
										...
										$theLines['getFieldContent'][$tcol] = trim(
											$this->sPS(
												'
													...
													return strftime(\'%A %e. %B %Y\', $this->internal[\'currentRow\'][\''
												. $tcol . '\']);
													break;
										'
											)
									...
									if (strstr($fC['type'], 'input')) {
										$theLines['getListHeader'][$tcol] =
											'<td><p>\' . $this->getFieldHeader_sortLink(\''
											. $tcol . '\') . \'</p></td>';
										$theLines['orderByList'][$tcol] = $tcol;
									}
						...
						$theLines['singleRows']['tstamp'] = trim(
							$this->sPS(
								'
															<tr>
						...
						$theLines['singleRows']['crdate'] = trim(
							$this->sPS(
								'
															<tr>
						...
						$this->addLocalLangFile(
							$ll, $pathSuffix . 'locallang.xml', 'Language labels for plugin "' . $cN . '"'
						);
						...
						$innerMainContent .= $this->sPS(
							'
								...
								if (is_numeric($this->piVars[\'showUid\'])) {	// If a single element should be displayed:
									...
									$this->internal[\'searchFieldList\'] = \'' . implode(
								',', $theLines['searchFieldList']
							) . '\';
									...
									$fullTable .= ' . ($config['list_default_listmode'] ? '$this->makeList($res);'
						...
						*/
						$this->addFileToFileArray(
							$config['plus_not_staticTemplate'] ? 'ext_typoscript_setup.txt'
								: $pathSuffix . 'static/setup.txt', $this->sPS(
								'
															plugin.' . $cN . ' {
								CMD =
								pidList =
						...
						if (!$config['plus_not_staticTemplate']) {
							$this->wizard->ext_tables[] = $this->sPS(
								'
																t3lib_extMgm::addStaticFile($_EXTKEY, \'' . $pathSuffix . 'static/\', \'' . addslashes(
									trim($config['title'])
								) . '\');
							'
							);
					...
					$this->addLocalConf(
						$ll, array('submit_button_label' => 'Click here to submit value'), 'submit_button_label', 'pi',
						$k, 1, 1
					);
				...
				$innerMainContent = $this->sPS(
					'
										...
										protected function tellWhatToDo($str) {
											return \'<a href="#" onClick="alert(\\\'Open the PHP-file \'.t3lib_extMgm::siteRelPath(\''
					. $extKey . '\').\'' . $pathSuffix . 'class.' . $cN . '.php and edit the function main()\nto change how the menu is rendered! It is pure PHP coding!\\\')">\'.$str.\'</a>\';
					}
				'
				);
				break;
...
',
			...
			// Add wizard icon
			$this->addFileToFileArray(
				$pathSuffix . 'ce_wiz.gif', t3lib_div::getUrl(t3lib_extMgm::extPath('kickstarter') . 'res/wiz.gif')
			);
			...
			$this->wizard->ext_tables[] = $this->sPS(
				...
				if (TYPO3_MODE === \'BE\') {
					$TBE_MODULES_EXT[\'xMOD_db_new_content_el\'][\'addElClasses\'][\'' . $cN
				. '_wizicon\'] = t3lib_extMgm::extPath($_EXTKEY) . \'pi' . $k . '/class.' . $cN . '_wizicon.php\';
				}
			'
			);
		}
