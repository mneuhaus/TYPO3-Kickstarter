...
class tx_kickstarter_section_module extends tx_kickstarter_sectionbase {
	...
	function render_wizard() {
		...
		if ($action[0] == 'edit') {
			...
			$subContent = $this->renderCheckBox(
					$ffPrefix . '[interface]', $piConf['interface']
				) . 'Allow other extensions to interface with function menu<br />';
		}
	...
	function render_extPart($k, $config, $extKey) {
		$WOP = '[module][' . $k . ']';
		$mN = ($config['position'] != '_MAIN' ? $config['position'] . '_' : '') . $this->returnName(
				$extKey, 'module', 'M' . $k
			);
		...
		$this->wizard->ext_tables[] = $this->sPS(
			...
			. '\');' . '
					' . $this->WOPcomment(
				'1. and 2. parameter is WOP:' . $WOP . '[position] , 3. parameter is WOP:' . $WOP . '[subpos]'
			) . '
		...
		$content = $this->sPS(
			...
			$MCONF[\'script\'] = \'_DISPATCH\';
				' . $this->WOPcomment(
				'WOP:' . $WOP . '[admin_only]: If the flag was set the value is "admin", otherwise "user,group"'
			) . '
		...
		$this->addLocalLangFile(
			$ll, $pathSuffix . 'locallang_mod.xml', 'Language labels for module "' . $mN . '" - header, description'
		);
...
<!-- ###BUTTON_GROUPS_RIGHT### -->';
		...
		$this->addFileToFileArray(
			$pathSuffix . 'moduleicon.gif',
			t3lib_div::getUrl(t3lib_extMgm::extPath('kickstarter') . 'res/notfound_module.gif')
		);
...
if (defined('TYPO3_MODE')
	&& isset($GLOBALS['TYPO3_CONF_VARS'][TYPO3_MODE]['XCLASS']['ext/kickstarter/sections/class.tx_kickstarter_section_module.php'])
) {
