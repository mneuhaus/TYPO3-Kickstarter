	...
	function render_extPart($k, $config, $extKey) {
		...
		$this->wizard->ext_tables[] = $this->sPS(
			'
					...
					\'' . addslashes($this->getSplitLabels_reference($config, 'title', 'moduleFunction.' . $cN)) . '\''
			. ($subPos ? ',
			...
			$indexContent = $this->sPS(
				'class ' . $cN . ' extends t3lib_extobjbase {
					...
					public function main() {
						...
						$menu=array();
						$menu[]=t3lib_BEfunc::getFuncCheck($this->pObj->id,"SET[' . $cN
				. '_check]",$this->pObj->MOD_SETTINGS["' . $cN . '_check"]).$LANG->getLL("checklabel");
						$theOutput.=$this->pObj->doc->spacer(5);
			...
			$indexContent = $this->sPS(
				'class ' . $cN . ' extends mod_user_task {
					...
					function overviewContent()	{
						...
						return \'<a href="index.php?SET[function]=' . $cN
				. '"  onClick="this.blur();"><img src="\'.$this->backPath.\'gfx/edit2.gif" style="float: left;"></a><div><a href="index.php?SET[function]='
				. $cN . '"  onClick="this.blur();">\'.$content.\'</a></div>\';
					}
...
if (defined('TYPO3_MODE')
	&& $TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/kickstarter/sections/class.tx_kickstarter_section_modulefunction.php']
) {
