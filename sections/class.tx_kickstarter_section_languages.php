	...
	function render_wizard() {
		...
		if ($action[0] == 'edit') {
			...
			while (list($k, $v) = each($this->wizard->languages)) {
				$key = $ffPrefix . '[' . $k . ']';
				$subContent .= $this->renderCheckBox($key, $piConf[$k]) . '<label for="field_' . md5($key) . '">'
					. $v . '</label><br />';
			}
			$lines[] = '<tr' . $this->bgCol(3) . '><td>' . $this->fw(
					$this->textSetup('Select which languages to setup:', $subContent)
				) . '</td></tr>';
		}
...
if (defined('TYPO3_MODE')
	&& $TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/kickstarter/sections/class.tx_kickstarter_section_languages.php']
) {
	include_once($TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/kickstarter/sections/class.tx_kickstarter_section_languages.php']);
}
