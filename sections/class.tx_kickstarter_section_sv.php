	...
	function render_wizard() {
		...
		if ($action[0] == 'edit') {
			...
			$lines = $this->catHeaderLines(
				$lines, $this->sectionID, $this->wizard->options[$this->sectionID],
				'<strong>Edit Service #' . $action[1] . '</strong>', $action[1]
			);
			...
			$subContent = '<strong>Title:</strong><br />' .
				$this->renderStringBox(
					$ffPrefix . '[title]', $piConf['title'] ? $piConf['title'] : $extKeyRec['title']
				);
			...
			$subContent = '<strong>Service type:</strong><br />' .
				$this->renderStringBox($ffPrefix . '[type]', $piConf['type'] ? $piConf['type'] : $extKeyRec['type'])
				. '<br />' .
			...
			$subContent = '<strong>Sub type(s) (comma list):</strong><br />' .
				$this->renderStringBox(
					$ffPrefix . '[subtype]', $piConf['subtype'] ? $piConf['subtype'] : $extKeyRec['subtype']
				) . '<br />' .
	...
	function render_extPart($k, $config, $extKey) {
		...
		$this->wizard->ext_localconf[] = $this->sPS(
			...
			t3lib_extMgm::addService($_EXTKEY, \'' . $config['type'] . '\',  \'' . $cN . '\',
				array(
					\'title\' => \'' . addslashes($config['title']) . '\',' . $this->WOPcomment(
				'	WOP:' . $WOP . '[title]'
			) . '
					\'description\' => \'' . addslashes($config['description']) . '\',' . $this->WOPcomment(
				'	WOP:' . $WOP . '[description]'
			) . '
...
if (defined('TYPO3_MODE')
	&& $TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/kickstarter/sections/class.tx_kickstarter_section_sv.php']
) {
