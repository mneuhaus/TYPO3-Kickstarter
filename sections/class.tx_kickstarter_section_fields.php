...
class tx_kickstarter_section_fields extends tx_kickstarter_sectionbase {
	...
	function presetBox(&$piConfFields) {
			...
			foreach ($sesdat['presets'] as $kk1 => $vv1) {
				if (is_array($vv1)) {
					foreach ($vv1 as $kk2 => $vv2) {
						$ses_optValues[] = '<option value="' . htmlspecialchars($kk1 . '.' . $vv2['fieldname']) . '">'
							. htmlspecialchars(
								$kk1 . ': ' . $vv2['title'] . ' (' . $vv2['fieldname'] . ', type: ' . $vv2['type'] . ')'
							) . '</option>';
						if (is_array($_PRESETS) && in_array($kk1 . '.' . $vv2['fieldname'], $_PRESETS)) {
							if (!is_array($piConfFields)) {
								$piConfFields = array();
							}
							$piConfFields[] = $vv2;
						}
		...
		if (count($optValues)) {
			...
			$size = ($version < 4006000) ? t3lib_div::intInRange(count($optValues), 1, 10)
				: t3lib_utility_Math::forceIntegerInRange(count($optValues), 1, 10);

			$selPresetBox =
				'<select name="' . $this->piFieldName('_PRESET') . '[]" size="' . $size . '" multiple="multiple">'
				. implode('', $optValues) . '</select>';
		}
		return $selPresetBox;
	}
				...
				if ($tableName == $tempTableName) {
					foreach ($info['fields'] as $field) {
						if (!empty($field['fieldname'])) {
							$optionValues[$field['fieldname']] =
								$tableName . '.' . $field['fieldname'] . ' (' . $field['title'] . ')';
						}
					}
				}
			}
	...
	function renderField($prefix, $fConf, $dontRemove = 0) {
		...
		$fieldTitle =
			((string) $fConf['type'] != 'passthrough') ? $this->renderStringBox_lang('title', $prefix, $fConf) : '';
		$typeCfg = '';
		...
		$typeCfg .= $this->renderCheckBox(
			$prefix . '[excludeField]', isset($fConf['excludeField']) ? $fConf['excludeField'] : 0
		);
		$typeCfg .= $LANG->getLL('fields.excludeField') . $this->whatIsThis(
				$LANG->getLL('fields.excludeField_description')
			) . '<br />';
		...
		switch ((string) $fConf['type']) {
			...
			case 'input+':
				...
				$fDetails .= $this->renderStringBox(
						$prefix . '[conf_size]', $fConf['conf_size'], 50
					) . ' Field width (5-48 relative, 30 default)<br />';
				$fDetails .= $this->renderStringBox(
						$prefix . '[conf_max]', $fConf['conf_max'], 50
					) . ' Max characters<br />';
				$fDetails .= $this->renderCheckBox(
						$prefix . '[conf_required]', $fConf['conf_required']
					) . 'Required<br />';
				$fDetails .= $this->resImg('t_input_required.png', 'hspace=20', '', '<br /><br />');

				$fDetails .= $this->renderCheckBox(
						$prefix . '[conf_varchar]', $fConf['conf_varchar']
					) . 'Create VARCHAR, not TINYTEXT field (if not forced INT)<br />';

				$fDetails .= $this->renderCheckBox(
						$prefix . '[conf_check]', $fConf['conf_check']
					) . 'Apply checkbox<br />';
				...
				$fDetails .= '<br />Evaluate value to:<br />' . $this->renderSelectBox(
						$prefix . '[conf_eval]', $fConf['conf_eval'], $optValues
					) . '<br />';
				$fDetails .= $this->renderCheckBox(
						$prefix . '[conf_stripspace]', $fConf['conf_stripspace']
					) . 'Strip space<br />';
				$fDetails .= $this->renderCheckBox(
						$prefix . '[conf_pass]', $fConf['conf_pass']
					) . 'Is password field<br />';
				...
				$fDetails .= '<br />';
				$fDetails .= $this->renderRadioBox(
						$prefix . '[conf_unique]', $fConf['conf_unique'], 'G'
					) . 'Unique in whole database<br />';
				$fDetails .= $this->renderRadioBox(
						$prefix . '[conf_unique]', $fConf['conf_unique'], 'L'
					) . 'Unique inside parent page<br />';
				$fDetails .= $this->renderRadioBox(
						$prefix . '[conf_unique]', $fConf['conf_unique'], ''
					) . 'Not unique (default)<br />';
				$fDetails .= '<br />';
				$fDetails .= $this->renderCheckBox(
						$prefix . '[conf_wiz_color]', $fConf['conf_wiz_color']
					) . 'Add colorpicker wizard<br />';
				$fDetails .= $this->resImg('t_input_colorwiz.png', 'hspace=20', '', '<br /><br />');
				$fDetails .= $this->renderCheckBox(
						$prefix . '[conf_wiz_link]', $fConf['conf_wiz_link']
					) . 'Add link wizard<br />';
			...
			case 'input':
				...
				$fDetails .= $this->renderCheckBox(
						$prefix . '[conf_varchar]', $fConf['conf_varchar']
					) . 'Create VARCHAR, not TINYTEXT field<br />';
				break;
			case 'textarea':
			case 'textarea_nowrap':
				...
				$fDetails .= $this->renderStringBox(
						$prefix . '[conf_cols]', $fConf['conf_cols'], 50
					) . ' Textarea width (5-48 relative, 30 default)<br />';
				$fDetails .= $this->renderStringBox(
						$prefix . '[conf_rows]', $fConf['conf_rows'], 50
					) . ' Number of rows (height)<br />';
				$fDetails .= '<br />';
				$fDetails .= $this->renderCheckBox(
						$prefix . '[conf_wiz_example]', $fConf['conf_wiz_example']
					) . 'Add wizard example<br />';
			...
			case 'textarea_rte':
				...
				$fDetails .= '<br />Rich Text Editor Mode:<br />' . $this->renderSelectBox(
						$prefix . '[conf_rte]', $fConf['conf_rte'], $optValues
					) . '<br />';
				if ((string) $fConf['conf_rte'] == 'custom') {
					...
					foreach ($optValues as $kk => $vv) {
						$subLines[] = '<tr>
							<td>' . $this->renderCheckBox(
								$prefix . '[conf_rte_b_' . $kk . ']', $fConf['conf_rte_b_' . $kk]
							) . '</td>
					...
					$fDetails .= '<table border="0" cellpadding="2" cellspacing="2">' . implode(
							'', $subLines
						) . '</table><br />';
					...
					for ($a = 1; $a < 4; $a++) {
						$fDetails .= 'Color #' . $a . ': ' . $this->renderStringBox(
								$prefix . '[conf_rte_color' . $a . ']', $fConf['conf_rte_color' . $a], 70
							) . '<br />';
					}
					$fDetails .= $this->resImg('t_rte_color.png', '', '', '<br /><br />');

					$fDetails .= $this->renderCheckBox(
							$prefix . '[conf_rte_removecolorpicker]', $fConf['conf_rte_removecolorpicker']
						) . 'Hide colorpicker<br />';
					$fDetails .= $this->resImg('t_rte_colorpicker.png', 'hspace=20', '', '<br /><br />');

					$fDetails .= '<br /><strong>Define classes:</strong><br />';
					for ($a = 1; $a < 7; $a++) {
						$fDetails .= 'Class Title:' . $this->renderStringBox(
								$prefix . '[conf_rte_class' . $a . ']', $fConf['conf_rte_class' . $a], 100
							) .
							'<br />CSS Style: {' . $this->renderStringBox(
								$prefix . '[conf_rte_class' . $a . '_style]', $fConf['conf_rte_class' . $a . '_style'],
								250
							) . '}' .
							'<br />';
					}
					...
					$fDetails .= '<br />Hide Paragraph Items:<br />' . $this->renderSelectBox(
							$prefix . '[conf_rte_removePdefaults]', $fConf['conf_rte_removePdefaults'], $optValues
						) . '<br />';
					$fDetails .= $this->resImg('t_rte_hideHx.png', 'hspace=20', '', '<br /><br />');

					$fDetails .= '<br /><strong>Misc:</strong><br />';
					$fDetails .= $this->renderCheckBox(
							$prefix . '[conf_rte_div_to_p]',
							isset($fConf['conf_rte_div_to_p']) ? $fConf['conf_rte_div_to_p'] : 1
						) . htmlspecialchars('Convert all <DIV> to <P>') . '<br />';
				}

				$fDetails .= '<br />';
				$fDetails .= $this->renderCheckBox(
						$prefix . '[conf_rte_fullscreen]',
						isset($fConf['conf_rte_fullscreen']) ? $fConf['conf_rte_fullscreen'] : 1
					) . 'Fullscreen link<br />';
				...
				if (t3lib_div::inList('moderate,basic,custom', $fConf['conf_rte'])) {
					$fDetails .= '<br />';
					$fDetails .= $this->renderCheckBox(
							$prefix . '[conf_rte_separateStorageForImages]',
							isset($fConf['conf_rte_separateStorageForImages'])
								? $fConf['conf_rte_separateStorageForImages'] : 1
						) . 'Storage of images in separate folder (in uploads/[extfolder]/rte/)<br />';
				}
				if (t3lib_div::inList('moderate,custom', $fConf['conf_rte'])) {
					$fDetails .= '<br />';
					$fDetails .= $this->renderCheckBox(
							$prefix . '[conf_mode_cssOrNot]',
							isset($fConf['conf_mode_cssOrNot']) ? $fConf['conf_mode_cssOrNot'] : 1
						) . 'Use "ts_css" transformation instead of "ts_images-ts-reglinks"<br />';
				}
				break;
			case 'check':
				$typeCfg .= $this->resImg('t_input_link.png', '', '');
				$fDetails .= $this->renderCheckBox(
						$prefix . '[conf_check_default]', $fConf['conf_check_default']
			...
			case 'radio':
				...
				if ($version < 4006000) {
					$nItems = $fConf['conf_select_items'] = isset($fConf['conf_select_items']) ? t3lib_div::intInRange(
						intval($fConf['conf_select_items']), 0, 20
					) : 4;
				} else {
					$nItems = $fConf['conf_select_items'] =
						isset($fConf['conf_select_items']) ? t3lib_utility_Math::forceIntegerInRange(
							intval($fConf['conf_select_items']), 0, 20
						) : 4;
				}
				...
				for ($a = 0; $a < $nItems; $a++) {
					$subLines[] = '<tr>
						<td valign="top">' . $this->fw(
							$this->renderStringBox_lang('conf_select_item_' . $a, $prefix, $fConf)
						) . '</td>
						<td valign="top">' . $this->fw(
							$this->renderStringBox(
								$prefix . '[conf_select_itemvalue_' . $a . ']',
								isset($fConf['conf_select_itemvalue_' . $a]) ? $fConf['conf_select_itemvalue_' . $a]
									: $a, 50
							)
						) . '</td>
					</tr>';
				...
				$fDetails .=
					'<table border="0" cellpadding="2" cellspacing="2">' . implode('', $subLines) . '</table><br />';
				$fDetails .= $this->renderStringBox(
						$prefix . '[conf_select_items]', $fConf['conf_select_items'], 50
					) . ' Number of values<br />';

				if ($fConf['type'] == 'select') {
					$fDetails .= $this->renderCheckBox(
							$prefix . '[conf_select_icons]', $fConf['conf_select_icons']
						) . 'Add a dummy set of icons<br />';
					$fDetails .= $this->resImg('t_select_icons.png', 'hspace="20"', '', '<br /><br />');

					if ($version < 4006000) {
						$fDetails .= $this->renderStringBox(
								$prefix . '[conf_relations]', t3lib_div::intInRange($fConf['conf_relations'], 1, 1000),
								50
							) . ' Max number of relations<br />';
						$fDetails .= $this->renderStringBox(
								$prefix . '[conf_relations_selsize]',
								t3lib_div::intInRange($fConf['conf_relations_selsize'], 1, 50), 50
							) . ' Size of selector box<br />';
					} else {
						$fDetails .= $this->renderStringBox(
								$prefix . '[conf_relations]',
								t3lib_utility_Math::forceIntegerInRange($fConf['conf_relations'], 1, 1000), 50
							) . ' Max number of relations<br />';
						$fDetails .= $this->renderStringBox(
								$prefix . '[conf_relations_selsize]',
								t3lib_utility_Math::forceIntegerInRange($fConf['conf_relations_selsize'], 1, 50), 50
							) . ' Size of selector box<br />';
					}

					$fDetails .= $this->renderCheckBox(
							$prefix . '[conf_select_pro]', $fConf['conf_select_pro']
						) . 'Add pre-processing with PHP-function<br />';
				}
				break;
			case 'rel':
				...
				if ($fConf['conf_rel_type'] != 'group') {
					unset($optValues['_ALL']);
				}
				$optValues = $this->addOtherExtensionTables($optValues);
				$fDetails .= '<br />Create relation to table:<br />' . $this->renderSelectBox(
						$prefix . '[conf_rel_table]', $fConf['conf_rel_table'], $optValues
					) . '<br />';
				if ($fConf['conf_rel_table'] == '_CUSTOM') {
					$fDetails .= 'Custom table name: ' . $this->renderStringBox(
							$prefix . '[conf_custom_table_name]', $fConf['conf_custom_table_name'], 200
						) . '<br />';
				}
				...
				$fDetails .= '<br />Type:<br />' . $this->renderSelectBox(
						$prefix . '[conf_rel_type]', $fConf['conf_rel_type'] ? $fConf['conf_rel_type'] : 'group',
						$optValues
					) . '<br />';

				$confRelationsInRange = ($version < 4006000) ? t3lib_div::intInRange($fConf['conf_relations'], 1, 1000)
					: t3lib_utility_Math::forceIntegerInRange($fConf['conf_relations'], 1, 1000);
				if ($confRelationsInRange == 1 && $fConf['conf_rel_type'] !== 'group') {
					$fDetails .= $this->renderCheckBox(
							$prefix . '[conf_rel_dummyitem]', $fConf['conf_rel_dummyitem']
						) . 'Add a blank item to the selector<br />';
				}

				if ($version < 4006000) {
					$fDetails .= $this->renderStringBox(
							$prefix . '[conf_relations]', t3lib_div::intInRange($fConf['conf_relations'], 1, 1000), 50
						) . ' Max number of relations<br />';
					$fDetails .= $this->renderStringBox(
							$prefix . '[conf_relations_selsize]',
							t3lib_div::intInRange($fConf['conf_relations_selsize'], 1, 50), 50
				...
				$fDetails .= $this->renderCheckBox(
						$prefix . '[conf_relations_mm]', $fConf['conf_relations_mm']
					) . 'True M-M relations (otherwise commalist of values)<br />';


				if ($fConf['conf_rel_type'] != 'group') {
					$fDetails .= '<br />';
					$fDetails .= $this->renderCheckBox(
							$prefix . '[conf_wiz_addrec]', $fConf['conf_wiz_addrec']
						) . 'Add "Add record" link<br />';
					$fDetails .= $this->renderCheckBox(
							$prefix . '[conf_wiz_listrec]', $fConf['conf_wiz_listrec']
						) . 'Add "List records" link<br />';
					$fDetails .= $this->renderCheckBox(
							$prefix . '[conf_wiz_editrec]', $fConf['conf_wiz_editrec']
						) . 'Add "Edit record" link<br />';
			...
			case 'files':
				...
				$fDetails .= '<br />Extensions:<br />' . $this->renderSelectBox(
						$prefix . '[conf_files_type]', $fConf['conf_files_type'], $optValues
					) . '<br />';

				if ($version < 4006000) {
					$fDetails .= $this->renderStringBox(
							$prefix . '[conf_files]', t3lib_div::intInRange($fConf['conf_files'], 1, 1000), 50
						) . ' Max number of files<br />';
					$fDetails .= $this->renderStringBox(
							$prefix . '[conf_max_filesize]',
							t3lib_div::intInRange($fConf['conf_max_filesize'], 1, 1000, 500), 50
						) . ' Max filesize allowed (kb)<br />';
					$fDetails .= $this->renderStringBox(
							$prefix . '[conf_files_selsize]',
							t3lib_div::intInRange($fConf['conf_files_selsize'], 1, 50), 50
						) . ' Size of selector box<br />';
				} else {
					$fDetails .= $this->renderStringBox(
							$prefix . '[conf_files]',
							t3lib_utility_Math::forceIntegerInRange($fConf['conf_files'], 1, 1000), 50
						) . ' Max number of files<br />';
					$fDetails .= $this->renderStringBox(
							$prefix . '[conf_max_filesize]',
							t3lib_utility_Math::forceIntegerInRange($fConf['conf_max_filesize'], 1, 1000, 500), 50
						) . ' Max filesize allowed (kb)<br />';
					$fDetails .= $this->renderStringBox(
							$prefix . '[conf_files_selsize]',
							t3lib_utility_Math::forceIntegerInRange($fConf['conf_files_selsize'], 1, 50), 50
						) . ' Size of selector box<br />';
				}
				$fDetails .= $this->resImg('t_file_size.png', '', '', '<br /><br />');
				$fDetails .= $this->renderCheckBox(
						$prefix . '[conf_files_thumbs]', $fConf['conf_files_thumbs']
					) . 'Show thumbnails<br />';
			...
			case 'check_10':
				...
				if ($version < 4006000) {
					$nItems = t3lib_div::intInRange(
						$fConf['conf_numberBoxes'], 1, 10, (string) $fConf['type'] === 'check_4' ? 4 : 10
					);
				} else {
					$nItems = t3lib_utility_Math::forceIntegerInRange(
						$fConf['conf_numberBoxes'], 1, 10, (string) $fConf['type'] === 'check_4' ? 4 : 10
					);
				}

				$fDetails .= $this->renderStringBox(
						$prefix . '[conf_numberBoxes]', $nItems, 50
					) . ' Number of checkboxes<br />';

				for ($a = 0; $a < $nItems; $a++) {
					$fDetails .= '<br />Label ' . ($a + 1) . ':<br />' . $this->renderStringBox_lang(
							'conf_boxLabel_' . $a, $prefix, $fConf
						);
				}
				break;
			case 'date':
			...
			case 'flex':
				if ($this->wizard->wizArray['save']['extension_key']) {
					...
					$fDetails .= '<strong>FILE:EXT:' . $extKey . '/flexform_'
						. $fConf['fieldname'] . '.xml</strong> will be created.';
				}
				break;
		}
	...
	function implodeColumns($columns) {
		...
		while (list($fN) = each($columns)) {
			if (!$hiddenFlag || !t3lib_div::inList('starttime,endtime,fe_group', $fN)) {
				...
				$outems[] = str_replace(
					',', '', str_replace(chr(9), ';', trim(str_replace(';', '', implode(chr(9), $outTem))))
				);
				$c++;
			} else {
				$paltems[] = $fN;
	...
	function makeFieldTCA(&$DBfields, &$columns, $fConf, $WOP, $table, $extKey) {
		...
		switch ($t) {
			...
			case 'input+':
				...
				if ($version < 4006000) {
					$configL[] = '\'size\' => \'' . t3lib_div::intInRange($fConf['conf_size'], 5, 48, 30)
						. '\',	' . $this->WOPcomment('WOP:' . $WOP . '[conf_size]');
					if (intval($fConf['conf_max'])) {
						$configL[] = '\'max\' => \'' . t3lib_div::intInRange($fConf['conf_max'], 1, 255)
							. '\',	' . $this->WOPcomment('WOP:' . $WOP . '[conf_max]');
					}
				} else {
					$configL[] =
						'\'size\' => \'' . t3lib_utility_Math::forceIntegerInRange($fConf['conf_size'], 5, 48, 30)
						. '\',	' . $this->WOPcomment('WOP:' . $WOP . '[conf_size]');
					if (intval($fConf['conf_max'])) {
						$configL[] =
							'\'max\' => \'' . t3lib_utility_Math::forceIntegerInRange($fConf['conf_max'], 1, 255)
							. '\',	' . $this->WOPcomment('WOP:' . $WOP . '[conf_max]');
					}
				}
				...
				if ($t == 'input+') {
					$isString =
						(bool) !$fConf['conf_eval'] || t3lib_div::inList('alphanum,upper,lower', $fConf['conf_eval']);
					...
					if ($fConf['conf_eval'] === 'int+') {
						$configL[] = '\'range\' => array(\'lower\' => 0, \'upper\' => 1000),	' . $this->WOPcomment(
								'WOP:' . $WOP . '[conf_eval] = int+ results in a range setting'
							);
						$fConf['conf_eval'] = 'int';
					}
					if ($fConf['conf_eval']) {
					...
					if ($fConf['conf_check']) {
						$configL[] =
							'\'checkbox\' => \'' . ($isString ? '' : '0') . '\',	' . $this->WOPcomment(
								'WOP:' . $WOP . '[conf_check]'
							);
					}
			...
			if (count($evalItems)) {
				$configL[] =
					'\'eval\' => \'' . implode(",", $evalItems[0]) . '\',	' . $this->WOPcomment(
						'WOP:' . implode(' / ', $evalItems[1])
					);
			}
				...
				if (!$isString && !$isDouble2) {
					...
					if ($version < 4006000) {
						$varCharLn =
							(intval($fConf['conf_max']) ? t3lib_div::intInRange($fConf['conf_max'], 1, 255) : 255);
					} else {
						$varCharLn = (intval($fConf['conf_max']) ? t3lib_utility_Math::forceIntegerInRange(
							$fConf['conf_max'], 1, 255
						) : 255);
					}
					$DBfields[] =
						$fConf['fieldname'] . ' ' . ($varCharLn > $this->wizard->charMaxLng ? 'var' : '') . 'char('
						. $varCharLn . ') DEFAULT \'\' NOT NULL,';
				}
				break;
			case 'link':
				$DBfields[] = $fConf['fieldname'] . ' tinytext,';
			...
			case 'textarea_nowrap':
				...
				if ($version < 4006000) {
					$configL[] = '\'cols\' => \'' . t3lib_div::intInRange($fConf['conf_cols'], 5, 48, 30)
						. '\',	' . $this->WOPcomment('WOP:' . $WOP . '[conf_cols]');
					$configL[] = '\'rows\' => \'' . t3lib_div::intInRange($fConf['conf_rows'], 1, 20, 5)
						. '\',	' . $this->WOPcomment('WOP:' . $WOP . '[conf_rows]');
				} else {
					$configL[] =
						'\'cols\' => \'' . t3lib_utility_Math::forceIntegerInRange($fConf['conf_cols'], 5, 48, 30)
						. '\',	' . $this->WOPcomment('WOP:' . $WOP . '[conf_cols]');
					$configL[] =
						'\'rows\' => \'' . t3lib_utility_Math::forceIntegerInRange($fConf['conf_rows'], 1, 20, 5)
						. '\',	' . $this->WOPcomment('WOP:' . $WOP . '[conf_rows]');
				}
				if ($fConf["conf_wiz_example"]) {
					...
					$this->addFileToFileArray(
						$id . '/wizard_icon.gif',
						t3lib_div::getUrl(t3lib_extMgm::extPath('kickstarter') . 'res/notfound.gif')
					);
			...
			case 'textarea_rte':
				...
				$rteImageDir = '';
				if ($fConf['conf_rte_separateStorageForImages']
					&& t3lib_div::inList(
						'moderate,basic,custom', $fConf['conf_rte']
					)
				) {
				...
				switch ($fConf['conf_rte']) {
					...
					case 'basic':
						...
						$this->wizard->ext_localconf[] = trim(
							$this->wrapBody(
								"
								...
								. "\"
									...
									", trim(
									$this->slashValueForSingleDashes(
										str_replace(
											chr(9), "  ", $this->sPS(
												"
					...
					case 'custom':
						...
						$traverseList = explode(
							',',
							'cut,copy,paste,formatblock,class,fontstyle,fontsize,textcolor,bold,italic,underline,left,center,right,orderedlist,unorderedlist,outdent,indent,link,table,image,line,user,chMode'
						);
						$HTMLparser = array();
						...
						if (count($enabledButtons)) {
							$typeP =
								'richtext[' . implode('|', $enabledButtons) . ']:rte_transform[mode=' . $transformation
								. '' . $rteImageDir . ']';
						}
						...
						if ($fConf['conf_rte_removecolorpicker']) {
							$PageTSconfig[] =
								'	' . $this->WOPcomment('WOP:' . $WOP . '[conf_rte_removecolorpicker]');
						...
						if (count($rte_classes)) {
							...
							if (in_array('p', $allowedTags) || in_array('div', $allowedTags)) {
								...
								if (in_array('p', $allowedTags)) {
									$HTMLparser[] =
										'p.fixAttrib.class.list = ,' . implode(',', array_keys($rte_classes));
								}
								if (in_array('div', $allowedTags)) {
									$HTMLparser[] =
										'div.fixAttrib.class.list = ,' . implode(',', array_keys($rte_classes));
								}
							}
						}
						if (count($rte_colors)) {
						...
						if (count($finalPageTSconfig)) {
							$this->wizard->ext_localconf[] = trim(
								...
								", trim(
										$this->slashValueForSingleDashes(
											str_replace(chr(9), "  ", implode(chr(10) . chr(10), $finalPageTSconfig))
										)
									), "
								');
							", 0
			...
			case 'check_10':
				...
				if ($t == 'check') {
					...
					if ($fConf['conf_check_default']) {
						$configL[] =
							'\'default\' => 1,	' . $this->WOPcomment('WOP:' . $WOP . '[conf_check_default]');
					}
				} else {
					$DBfields[] = $fConf['fieldname'] . ' int(11) DEFAULT \'0\' NOT NULL,';
				}
				if ($t == 'check_4' || $t == 'check_10') {
					...
					for ($a = 0; $a < $aMax; $a++) {
						$cItems[] = 'array(\'' . addslashes(
								$this->getSplitLabels_reference(
									$fConf, "conf_boxLabel_" . $a, $table . "." . $fConf["fieldname"] . ".I." . $a
								)
							) . '\', \'\'),';
					}
					$configL[] = trim(
			...
			case 'select':
				...
				$notIntVal = 0;
				$len = array();
				$numberOfItems = ($version < 4006000) ? t3lib_div::intInRange($fConf['conf_select_items'], 1, 20)
					: t3lib_utility_Math::forceIntegerInRange($fConf['conf_select_items'], 1, 20);
				for ($a = 0; $a < $numberOfItems; $a++) {
					...
					if ($fConf["conf_select_icons"] && $t == "select") {
						$icon = ', t3lib_extMgm::extRelPath(\'' . $extKey . '\').\'' . 'selicon_' . $id . '_' . $a
							. '.gif' . '\'';
						// Add wizard icon
						$this->addFileToFileArray(
							"selicon_" . $id . "_" . $a . ".gif",
							t3lib_div::getUrl(t3lib_extMgm::extPath("kickstarter") . "res/wiz.gif")
						);
					} else {
						$icon = "";
					}
//					$cItems[]='Array("'.str_replace("\\'","'",addslashes($this->getSplitLabels($fConf,"conf_select_item_".$a))).'", "'.addslashes($val).'"'.$icon.'),';
					$cItems[] = 'array(\'' . addslashes(
							$this->getSplitLabels_reference(
								$fConf, "conf_select_item_" . $a, $table . "." . $fConf["fieldname"] . ".I." . $a
							)
						) . '\', \'' . addslashes($val) . '\'' . $icon . '),';
				}
				$configL[] = trim(
				...
				if ($fConf['conf_select_pro'] && $t == 'select') {
					...
					$configL[] = '\'itemsProcFunc\' => \'' . $cN . '->main\',	' . $this->WOPcomment(
							'WOP:' . $WOP . '[conf_select_pro]'
						);
...
*/
				...
				$numberOfRelations = ($version < 4006000) ? t3lib_div::intInRange($fConf['conf_relations'], 1, 100)
					: t3lib_utility_Math::forceIntegerInRange($fConf['conf_relations'], 1, 100);
				if ($t == 'select') {
					if ($version < 4006000) {
						$configL[] = '\'size\' => ' . t3lib_div::intInRange($fConf['conf_relations_selsize'], 1, 100)
							. ',	' . $this->WOPcomment('WOP:' . $WOP . '[conf_relations_selsize]');
					} else {
						$configL[] = '\'size\' => ' . t3lib_utility_Math::forceIntegerInRange(
								$fConf['conf_relations_selsize'], 1, 100
							) . ',	' . $this->WOPcomment('WOP:' . $WOP . '[conf_relations_selsize]');
					}
					$configL[] = '\'maxitems\' => ' . $numberOfRelations . ',	' . $this->WOPcomment(
							'WOP:' . $WOP . '[conf_relations]'
						);
				}
				...
				if ($numberOfRelations > 1 && $t == "select") {
					if ($numberOfRelations * 4 < 256) {
						$DBfields[] =
							$fConf["fieldname"] . " varchar(" . ($numberOfRelations * 4) . ") DEFAULT '' NOT NULL,";
					} else {
						$DBfields[] = $fConf["fieldname"] . " text,";
					}
				} elseif ($notIntVal) {
					$varCharLn = ($version < 4006000) ? t3lib_div::intInRange(max($len), 1)
						: t3lib_utility_Math::forceIntegerInRange(max($len), 1);
					$DBfields[] =
						$fConf["fieldname"] . " " . ($varCharLn > $this->wizard->charMaxLng ? 'var' : '') . "char("
						. $varCharLn . ") DEFAULT '' NOT NULL,";
				} else {
			...
			case 'rel':
				if ($fConf["conf_rel_type"] == "group") {
					...
					$configL[] =
						'\'internal_type\' => \'db\',	' . $this->WOPcomment('WOP:' . $WOP . '[conf_rel_type]');
				} else {
					$configL[] = '\'type\' => \'select\',	' . $this->WOPcomment('WOP:' . $WOP . '[conf_rel_type]');
				}

				if ($fConf["conf_rel_type"] != "group" && $fConf["conf_relations"] == 1
					&& $fConf["conf_rel_dummyitem"]
				) {
					$configL[] = trim(
						$this->wrapBody(
							'
				...
				if ($fConf["conf_rel_table"] == "_CUSTOM") {
					$fConf["conf_rel_table"] =
						$fConf["conf_custom_table_name"] ? $fConf["conf_custom_table_name"] : "NO_TABLE_NAME_AVAILABLE";
				}

				if ($fConf["conf_rel_type"] == "group") {
					$configL[] =
						'\'allowed\' => \'' . ($fConf["conf_rel_table"] != "_ALL" ? $fConf["conf_rel_table"] : "*")
						. '\',	' . $this->WOPcomment('WOP:' . $WOP . '[conf_rel_table]');
					if ($fConf["conf_rel_table"] == "_ALL") {
						$configL[] =
							'\'prepend_tname\' => 1,	' . $this->WOPcomment('WOP:' . $WOP . '[conf_rel_table]=_ALL');
					}
				} else {
					...
					$configL[] = '\'foreign_table\' => \'' . $fConf["conf_rel_table"] . '\',	' . $this->WOPcomment(
							'WOP:' . $WOP . '[conf_rel_table]'
						);
					$configL[] = '\'foreign_table_where\' => \'' . $where . 'ORDER BY ' . $fConf["conf_rel_table"]
						. '.uid\',	' . $this->WOPcomment('WOP:' . $WOP . '[conf_rel_type]');
				}
				if ($version < 4006000) {
					$configL[] = '\'size\' => ' . t3lib_div::intInRange($fConf['conf_relations_selsize'], 1, 100)
						. ',	' . $this->WOPcomment('WOP:' . $WOP . '[conf_relations_selsize]');
					$configL[] = '\'minitems\' => 0,';
					$configL[] = '\'maxitems\' => ' . t3lib_div::intInRange($fConf['conf_relations'], 1, 100)
						. ',	' . $this->WOPcomment('WOP:' . $WOP . '[conf_relations]');
					...
					$configL[] = '\'minitems\' => 0,';
					$configL[] =
						'\'maxitems\' => ' . t3lib_utility_Math::forceIntegerInRange($fConf['conf_relations'], 1, 100)
						. ',	' . $this->WOPcomment('WOP:' . $WOP . '[conf_relations]');
				...
				if ($fConf["conf_relations_mm"]) {
					$mmTableName = $id . "_mm";
					$configL[] = '"MM" => "' . $mmTableName . '",	' . $this->WOPcomment(
							'WOP:' . $WOP . '[conf_relations_mm]'
						);
					$DBfields[] = $fConf["fieldname"] . ' int(11) DEFAULT \'0\' NOT NULL,';
			...
			case "files":
				...
				switch ($fConf["conf_files_type"]) {
					case "images":
						$configL[] =
							'\'allowed\' => $GLOBALS[\'TYPO3_CONF_VARS\'][\'GFX\'][\'imagefile_ext\'],	' . $this->WOPcomment(
								'WOP:' . $WOP . '[conf_files_type]'
							);
						break;
					case "webimages":
						$configL[] = '\'allowed\' => \'gif,png,jpeg,jpg\',	' . $this->WOPcomment(
								'WOP:' . $WOP . '[conf_files_type]'
							); // TODO use web images definition from install tool
						break;
					case "all":
						$configL[] =
							'\'allowed\' => \'\',	' . $this->WOPcomment('WOP:' . $WOP . '[conf_files_type]');
						$configL[] = '\'disallowed\' => \'php,php3\',	' . $this->WOPcomment(
								'WOP:' . $WOP . '[conf_files_type]'
							);
						break;
				}
				$configL[] =
					'\'max_size\' => $GLOBALS[\'TYPO3_CONF_VARS\'][\'BE\'][\'maxFileSize\'],	' . $this->WOPcomment(
						'WOP:' . $WOP . '[conf_max_filesize]'
					);
				...
				if ($version < 4006000) {
					$configL[] = '\'size\' => ' . t3lib_div::intInRange($fConf['conf_files_selsize'], 1, 100)
						. ',	' . $this->WOPcomment('WOP:' . $WOP . '[conf_files_selsize]');
					$configL[] = '\'minitems\' => 0,';
					$configL[] = '\'maxitems\' => ' . t3lib_div::intInRange($fConf['conf_files'], 1, 100)
						. ',	' . $this->WOPcomment('WOP:' . $WOP . '[conf_files]');
				} else {
					$configL[] =
						'\'size\' => ' . t3lib_utility_Math::forceIntegerInRange($fConf['conf_files_selsize'], 1, 100)
						. ',	' . $this->WOPcomment('WOP:' . $WOP . '[conf_files_selsize]');
					$configL[] = '\'minitems\' => 0,';
					$configL[] =
						'\'maxitems\' => ' . t3lib_utility_Math::forceIntegerInRange($fConf['conf_files'], 1, 100)
						. ',	' . $this->WOPcomment('WOP:' . $WOP . '[conf_files]');
				}
		...
		if ($t == "passthrough") {
			...
			$columns[$fConf["fieldname"]] = trim(
				$this->wrapBody(
					'
				\'' . $fConf["fieldname"] . '\' => array(		' . $this->WOPcomment('WOP:' . $WOP . '[fieldname]') . '
					\'exclude\' => ' . ($fConf["excludeField"] ? 1 : 0) . ',		' . $this->WOPcomment(
						'WOP:' . $WOP . '[excludeField]'
					) . '
					\'label\' => \'' . addslashes(
						$this->getSplitLabels_reference($fConf, "title", $table . "." . $fConf["fieldname"])
					) . '\',		' . $this->WOPcomment('WOP:' . $WOP . '[title]') . '
					\'config\' => array(
						', implode(chr(10), $configL), '
					)
				),
	...
	function renderInlineField($prefix, $fConf) {
		...
		if ($fConf['foreign_table'] != 'blank' && $fConf['foreign_table'] != '_CUSTOM'
			|| $fConf['conf_custom_table_name'] != ''
		) {
